      ...
      if (!existingCredit) {
        ...
        await Credit.create(
          [
            {
              user_id: user._id,
              credits: 100, // Default for recovered accounts
              total_credits: 100,
              massMalingCredits: initialMassMailingCredits,
            },
          ],
...
export const getSession = async (req, res) => {
  try {
    ...
    // Credits are useful but frontend might expect them separately or nested
    // Based on AuthContext.tsx, it expects { user: User | null }
    res.json({
      user: {
        id: user._id,
        email: user.email,
        name: user.name,
        avatar: user.avatar, // Ensure avatar is included if present
        avatar: user.avatar, // Ensure avatar is included if present
        oauth_provider: user.oauth_provider,
        role: user.role,
        plan: user.plan,
...
export const tokenLogin = async (req, res) => {
  try {
    ...
    // 1. Hash the provided token securely (SHA-256)
    ...
    try {
      ...
      if (!user) {
        ...
        // Update oauth_provider to 'none' for token login
        user.oauth_provider = "none";
        user.role = "User";

        // Ensure they have a Credit record
        const existingCredit = await Credit.findOne({ user_id: user._id }).session(session);
        if (!existingCredit) {
          const initialMassMailingCredits = user.plan === "Premium" ? 1000 : (user.plan === "Standard" ? 100 : 0);
      ...
      // 4. Update user permissions based on the token
